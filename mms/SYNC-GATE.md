...
| O11 | C01.06 | `mms/adapter/test_default_model.mjs` | 4 项全过：会话里切模型不再写默认；`/default-model` 只接受菜单发来的完整选择，写入默认；卸载插件恢复上游行为 | 不证明上游只有这一处写默认，由 L9 兜底 |
...
| L9 | C01.06 | W1 同一实例：`session/selectModel` 把会话切到另一个模型 → 读 settings.yaml → `/default-model` 设成它 → 再读 | 切模型后默认不变；显式设置后默认变成新值 | 不花 token。mutation（2026-09-19）：去掉插件后「切模型后默认不变」变红 |
