...
最后核对：2026-09-18。fork `origin/mms`，upstream `deepseek-ai/deepseek-harness`，当前 `ahead=4 / behind=0`。来源基线 MMS `main 4.23.8` / `dev 5.1.11`。
...
已经在 fork 里落地的三个能力（commit `e530fa4fd0`）：项目按名字/路径搜索并确认实际 cwd、常驻 effort 直接选档、MMS 模型路由与凭据隔离。
...
**必跑验收子集**：每次同步之后必须重跑的那批，覆盖已经迁进来的每一个能力项。现在还没有这份清单，**建这份清单是接手后的前三件事之一**。它的形状是：
...
1. **建"必跑验收子集"**（第四节那张表），把已迁的三个能力先写进去。没有它，上游同步就是盲合。
2. **补救那 25 处上游就地修改**，优先共享 `locale` 和两个 ui 包。趁只有 302 行。同时把 `C24.02` 提到最前面。
3. **跑桶 A 的验收**，把"dsh 原生已达标"的项按 A 类流程登记。跑完你会知道真正要迁的到底有多少——这个数字会让后面所有决定变容易。

然后才开始桶 B 的第 1 项（手机远程）。
