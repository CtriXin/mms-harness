...
Double-click the launcher to start or reuse the service and open its browser. `start/stop/status/open` manage only that instance; stopping retains sessions. The default listener is local-only `127.0.0.1:3092`; phone access remains in the migration inventory.
